...
;; org-bluesky-atproto 用に com-junkawasaki/root superproject の
;; manifest/edn-datomize.bb 系譜（gftdcojp/net-kotobase Phase 2 pilot 版）から
;; 移植（Phase 3 fan-out, ADR: superproject 側の "EDN datomize" ワークストリーム）。
;; schema-path をこの repo のルート schema.edn に向けている以外はロジック同一
;; ＋ wrap-generic モード（既に名前空間付きキーを持つ map 用）。
;;
